...
from utils.wordle_helpers import check_answer, get_emoji_word, get_word, get_wordle_stats, wordle_exception, valid_word, count_wordle_field_length
...
async def wait_for_answer(interaction, word, word_len, word_set, emoji_letters, alphabet):
        ...
        while not correct:
            ...
            if msg:
                if guess.split(' ')[0] == 'bg':
                    ...
                    await interaction.followup.send(content=tag_user, embed=embed_wordle(wordle_invalid_word))
                else:
                    ...
                    if not wrong_len:
                        ...
                        emoji_alphabet = get_emoji_word(
                            ...
                            emoji_letters)
                        current_guess_string = f'{emoji_guess_word} | {squares_response}'
                        old_field_length = count_wordle_field_length(past_guesses, emoji_guess_word, squares_response, fail_count, word_len)
                    ...
                    if correct:
                        ...
                        await interaction.followup.send(content=tag_user, embed=embed_wordle(wordle_success))
                        return
                    elif wrong_len:
                        wordle_bad_word = {
                            player_title: f'Your guesses must be {word_len} letters long! Try again!'}
                        await interaction.followup.send(content=tag_user, embed=embed_wordle(wordle_bad_word))
                    elif (fail_count == word_len + 1):
                        wordle_fail = {
                            player_title: f'{common_response_text}',
                            'Incorrect!': f'The correct word was {emoji_correct_word}'}
                        await interaction.followup.send(content=tag_user, embed=embed_wordle(wordle_fail))
                        break
                    else:
                        wordle_guess_again = {
                            player_title: f'{common_response_text}',
                            'Unused Letters': emoji_alphabet}
                        await interaction.followup.send(content=tag_user, embed=embed_wordle(wordle_guess_again))
    except asyncio.TimeoutError:
        wordle_timeout_error = {
            'A wordley timeout': f'Guess quicker next time!\nThe word was {emoji_correct_word}'}
        await interaction.followup.send(content=tag_user, embed=embed_wordle(wordle_timeout_error))
...
class Wordle(commands.GroupCog, name='wordle'):
    ...
    async def wordle_play(self, interaction: discord.Interaction, word_length: int=5) -> None:
        try:
            ...
            await wait_for_answer(interaction, word, word_length, WORD_SET, REGIONAL_INDICATOR_LETTERS, ALPHABET)
