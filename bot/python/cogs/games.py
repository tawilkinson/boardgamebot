...
class Games(commands.GroupCog, name="games"):
    ...
    @app_commands.command(
        name="search",
        description="Fetches game info from \
    ...
    async def game_search(self, interaction: discord.Interaction, game: str) -> None:
        ...
        search_game = await search_web_board_game_data(game_str, message, interaction)
        if logger.level >= 10:
    ...
    @app_commands.command(
        name="tabletopia",
        description="Tabletopia has over 1600 games, so prints a link to the all games page on Tabletopia",
    )
    ...
    @commands.command(
        help="Lists the help for command category `games`", pass_context=True
    )
